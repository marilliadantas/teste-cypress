...
Cypress.Commands.add('login_api', (username, password) => {
  cy.api({
    ...
    return response.body.token // retorna o token
  })
})

// Através do método then, é possível validar a response da requisição
...
Cypress.Commands.add('find_user_by_username_api', (username) => {
  ...
  //     cy.find_user_by_username_api(data.username).then((user) => Cypress.env('auth.user.id', user))
  //   })
  })